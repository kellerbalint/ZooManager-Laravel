...
@section('content')
    <form action="{{ route('createAnimal') }}" method="post" enctype="multipart/form-data">
        @csrf
        ...
        <input name="species" type="text" value="{{ old('species') }}" class="@error('species') is-invalid @enderror">
        @error('species')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
        ...
        <p>
            ...
            <input type="checkbox" name="is_predator" id="" @checked(old('is_predator'))>
        </p>
        ...
        <p>
            ...
            <input name="born_at" type="date" value="{{ old('born_at') }}"
